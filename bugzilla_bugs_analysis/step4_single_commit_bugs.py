...
"""
================================================================================
STEP 4: SINGLE FIXING COMMIT + REGRESSOR FILE MATCHING (with test file filtering)
================================================================================
...
NEW: Filters out test files from file lists and provides statistics
"""
...
class SingleCommitRegressorMatcher:
    ...
    # Test file patterns - common patterns in Mozilla codebase
    TEST_FILE_PATTERNS = [
        r'/test/',           # directory named 'test'
        r'/tests/',          # directory named 'tests'
        r'/testing/',        # directory named 'testing'
        r'/mochitest/',      # mochitest directory
        r'/reftest/',        # reftest directory
        r'/xpcshell/',       # xpcshell test directory
        r'/gtest/',          # gtest directory
        r'/crashtest/',      # crashtest directory
        r'test_.*\..*$',     # files starting with test_
        r'.*_test\..*$',     # files ending with _test
        r'.*_tests\..*$',    # files ending with _tests
        r'.*\.test\..*$',    # files with .test. in name
        r'/browser_.*\.js$', # browser test files
        r'/unit/.*\.js$',    # unit test files
        r'\.mochitest\.', # mochitest files
        r'\.reftest\.',   # reftest files
        r'/gtests/',         # gtest plural
    ]
    ...
    def _filter_test_files(self, files: List[str]) -> Tuple[List[str], List[str], int]:
        """
        Filter test files from file list.
        
        Returns:
            Tuple of (non_test_files, test_files, test_count)
        """
        non_test = []
        test_files = []
        ...
        return non_test, test_files, len(test_files)
    ...
    def get_matching_regressor_commits(self, regressor_commits: List[Dict], 
        ...
        """Get regressor commits that modified same files as the fix (excluding test files)"""
        ...
        for commit in regressor_commits:
            # Filter test files from regressor commit
            all_reg_files = commit.get('files', [])
            reg_files, reg_test_files, reg_test_count = self._filter_test_files(all_reg_files)
            
            # Check overlap with non-test files only
            overlap = list(fixing_set & set(reg_files))
            if overlap:
                matching.append({
                    ...
                    'author': commit.get('author', ''),
                    'pushdate': commit.get('pushdate', ''),
                    'files_modified': reg_files,  # Only non-test files
                    'test_files_count': reg_test_count,
                    'test_files': reg_test_files,
    ...
    def process_single_bug(self, bug_id: str, bug_data: Dict, idx: int, total: int) -> Dict:
        ...
        # Single commit - filter test files
        result['category'] = 'single_commit'
        fix = fixing_commits[0]
        all_fixing_files = fix.get('files', [])
        fixing_files, fixing_test_files, fixing_test_count = self._filter_test_files(all_fixing_files)
        ...
        result['fixing_commit'] = {
            ...
            'files': fixing_files,  # Only non-test files
            'file_count': len(fixing_files),
            'test_files_count': fixing_test_count,
            'test_files': fixing_test_files,
            'total_files_count': len(all_fixing_files),
            'components': fix.get('components', [])
        }
        
        # Find matching regressor commits (using non-test files only)
        ...
        # Enhanced logging with test file info
        test_info = f" ({fixing_test_count} test)" if fixing_test_count > 0 else ""
        self._safe_print(
            f"[{idx}/{total}] Bug {bug_id}: ✓ Single ({fix['short_node']}) "
            f"- {len(fixing_files)} files{test_info} → {total_matching} matches [{source}]"
        )
        return result
    ...
    def filter_and_match(self) -> Dict:
        """Main processing"""
        print("=" * 70)
        print("STEP 4: SINGLE COMMIT + REGRESSOR MATCHING (with test filtering)")
        print("=" * 70 + "\n")
        ...
        test_file_stats = {
            'bugs_with_test_files_in_fix': 0,
            'total_test_files_filtered': 0,
            'bugs_with_test_files_in_regressors': 0
        }
            ...
            for future in as_completed(futures):
                ...
                try:
                    ...
                    with self.result_lock:
                        ...
                        # Track test file statistics
                        if result['category'] == 'single_commit':
                            fix_test_count = result['fixing_commit'].get('test_files_count', 0)
                            if fix_test_count > 0:
                                test_file_stats['bugs_with_test_files_in_fix'] += 1
                                test_file_stats['total_test_files_filtered'] += fix_test_count
                            
                            # Check regressor commits for test files
                            for reg_analysis in result.get('regressor_analysis', []):
                                for match in reg_analysis.get('matching_commits', []):
                                    if match.get('test_files_count', 0) > 0:
                                        test_file_stats['bugs_with_test_files_in_regressors'] += 1
                                        break
        ...
        results = {
            ...
            'summary': {
                ...
                'test_file_statistics': test_file_stats
            },
    ...
    def _print_summary(self, results: Dict):
        ...
        test_stats = s.get('test_file_statistics', {})
        if test_stats:
            print(f"\nTest File Statistics:")
            print(f"  Bugs with test files in fix: {test_stats.get('bugs_with_test_files_in_fix', 0)}")
            print(f"  Total test files filtered: {test_stats.get('total_test_files_filtered', 0)}")
            print(f"  Bugs with test files in regressors: {test_stats.get('bugs_with_test_files_in_regressors', 0)}")
...
def main():
    ...
    if 'error' not in results:
        ...
        test_stats = s.get('test_file_statistics', {})
        if test_stats:
            print(f"\n  Test files filtered: {test_stats.get('total_test_files_filtered', 0)}")
